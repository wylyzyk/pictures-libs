 ...
 */
const onToFeedback = () => {
  window.open(FEEDBACK_URL, "_black");
};
</script>
  ...
  <div class="fixed bottom-10 right-2">
    ...
    <Popover class="guide-feedback flex items-center" placement="top-left">
      <template #reference>
        ...
        >
          <SvgIcon name="feedback" class="w-2 h-2" fillClass="fill-zinc-900 dark:fill-zinc-200 group-hover:fill-main" />
        </div>
      </template>

      <div class="w-[140px] overflow-hidden">
        <div class="flex items-center p-1 cursor-pointer rounded hover:bg-zinc-100/60 dark:bg-zinc-800">
          <SvgIcon name="feedback" class="w-1.5 h-1.5 mr-1" fillClass="fill-zinc-900 dark:fill-zinc-300" />
          <span class="text-zinc-800 dark:text-zinc-300 text-sm" @click="onToFeedback">立即吐槽</span>
        </div>
      </div>
    </Popover>
  </div>
